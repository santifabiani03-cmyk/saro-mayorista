...
export default function Header({ config }) {
  ...
  // En las fichas queda oculto hasta saber que el producto es mayorista (así las
  // paletas no muestran el aviso ni por un instante al cargar).
  const esFicha = pathname?.startsWith('/producto/')
  const verCompraMinima = config.mostrarCompraMinima && !esMinorista
    && (!esFicha || fichaModo === 'mayorista')
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl border-2 border-saro-blue/20 text-saro-blue hover:bg-saro-blue hover:text-white hover:border-saro-blue transition duration-200 text-sm font-semibold btn-press"
              title="¿Cómo comprar?"
            >
              ¿Cómo comprar?
            </button>
            ...
            {!isLanding && (
              <button
                ...
                className="relative flex items-center gap-2 bg-saro-dark hover:bg-saro-blue text-white px-4 py-2.5 rounded-xl font-semibold text-sm transition duration-200 shadow-md shadow-saro-dark/20 btn-press"
              >
