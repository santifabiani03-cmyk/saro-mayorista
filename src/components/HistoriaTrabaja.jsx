...
function Collapsible({ subtitle, title, accent, open, onToggle, children }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100/80 shadow-card overflow-hidden">
      ...
      <div className={`grid transition-all duration-300 ease-in-out ${open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
        <div className="overflow-hidden">
          <div className="px-5 sm:px-6 pb-6 pt-5 border-t border-gray-100/80">
            {children}
          </div>
        </div>
      </div>
...
export default function HistoriaTrabaja({ whatsappNumber }) {
  ...
  return (
    <section id="trabaja" className="relative bg-[#FAFBFC] py-16 sm:py-24 scroll-mt-4">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 grid md:grid-cols-2 gap-4 sm:gap-6 items-start">

        {/* Izquierda: Nuestra historia + política */}
        <Collapsible
          subtitle="La marca"
          title="Nuestra historia y política"
          accent="text-saro-blue"
          open={openHist}
          onToggle={() => setOpenHist(o => !o)}
        >
        ...
        {/* Derecha: Trabajá con nosotros (formulario) */}
        <Collapsible
          subtitle="¿Tenés un comercio?"
          title="Trabajá con nosotros"
          accent="text-saro-accent"
          open={openForm}
          onToggle={() => setOpenForm(o => !o)}
        >
