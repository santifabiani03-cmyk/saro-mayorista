...
// Catálogo mayorista: precios por mayor y compra mínima.
export default function RopaYAccesoriosMayoristaPage() {
  const all = JSON.parse(fs.readFileSync(CATALOG_FILE, 'utf-8'))
  const products = all.filter(p => p.categoria !== 'paleta')
  return <CatalogView products={products} kind="ropa" modo="mayorista" />
}
